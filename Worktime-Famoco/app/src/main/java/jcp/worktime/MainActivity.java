...
import jcp.worktime.composant.DateTimeUtils;
import jcp.worktime.composant.DialogTexte;
import jcp.worktime.composant.Location;
import jcp.worktime.composant.SharedPreferencesHelper;
import jcp.worktime.dto.PostPassage;
import jcp.worktime.dto.ScanSuccess;
import jcp.worktime.json.JsonTaskEmployees;
import jcp.worktime.json.JsonTaskScanSuccess;
import jcp.worktime.outil.Format;
    ...
    private void ReadingTag(String hexTagId) {
        try {
            ...
            if (hexTagId.length() > 0 && isTagReadingAllowed)
            {
                ...
                Toast.makeText(getApplicationContext(), R.string.time_warning, Toast.LENGTH_LONG).show();
            }
            scanEnCours = false;
