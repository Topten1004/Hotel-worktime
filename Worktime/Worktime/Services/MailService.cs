...
namespace Worktime.Services
{
    public class MailService : IMailService
    {
        ...
        public async Task SendForgotPassword(ForgotPasswordVM request)
        {
            string FilePath = Directory.GetCurrentDirectory() + @"\wwwroot\mail\ForgotPassword.html";
            ...
            string confirmLink = "<a href='>" + "https://jcp.worktime.nc/Home/ResetPassword?email=" + request.Email + "&password=" + request.Password + "'>Modifier mon mot de passe</a>";

            email.Subject = $"WORKTIME - Avez-vous besoin de réinitialiser votre mot de passe ?";

            MailText = MailText.Replace("[ConfirmLink]", confirmLink);

            var plain = new TextPart(MimeKit.Text.TextFormat.Plain) { Text = $"WORKTIME - Avez-vous besoin de réinitialiser votre mot de passe ? - https://jcp.worktime.nc" };
