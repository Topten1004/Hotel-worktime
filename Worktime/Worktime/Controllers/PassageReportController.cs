    ...
    public class PassageReportController : Controller
    {
        ...
        public async Task<IActionResult> Index(string Month, string Year, int page = 1, int pageSize = 20)
        {
            ...
            foreach (var item in model)
            {
                ...
                var monthlyPassages = item.Passages.Where(x => x.LogTime.Month == intMonth && x.LogTime.Year == intYear).OrderByDescending(x => x.LogTime).ToList();
            ...
            totalItems = totalItems.OrderByDescending(x => x.LogTime).ToList();
        ...
        public async Task<IActionResult> Details(string? SSN, DateTime? day)
        {
            ...
            foreach (var item in model)
            {
                ...
                var passageList = item.Passages.OrderByDescending( x => x.LogTime).ToList();
